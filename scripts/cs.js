...
(function ($) {
 		...
 		_getOCRServer().done(function (serverId) {
  		 ...
  		 let	 request = {
				 ...
				 chrome.runtime.sendMessage({
					 ...
					 console.log('answers of srver data',responseData);
					 var result;
		...
		_getOCRServer().done(function (serverId) {
			...
			$.ajax({
				...
				success: function (data) {
					console.log('answers of srver data',data.ParsedResults[0].TextOverlay);
					var result;
					data = data || {};
					// retry if any error condition is met and if any servers are still available
	...
	function openGoogleTransatePage() {
		...
		let $text = `https://translate.google.com/?text=${translatedText}&hl=${userLang}&langpair=auto|${userLang}&tbb=1`;
	...
	var OCRTranslator = {
		...
		addIconToTranslateButton: function () {
			...
			$translateButtonImg.attr('src',chrome.runtime.getURL("images/translate.png"));
		},
